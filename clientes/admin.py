...
from .models import Usuario, PerfilEmpresa, LogLogin, SessaoAtiva
...
class UsuarioAdmin(UserAdmin):
    ...
    fieldsets = (
        ...
        (_('Informacoes Pessoais'), {
            'fields': ('nome_completo', 'telefone', 'cpf', 'foto')
        }),
        (_('Preferencias'), {
            'fields': ('idioma_preferido', 'notificacoes_email', 'notificacoes_sms', 'two_factor_enabled')
        }),
        (_('Permissoes'), {
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('email', 'nome_completo', 'password1', 'password2', 'is_active', 'is_staff'),
        }),
    )
...
class LogLoginAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['usuario', 'ip_address', 'user_agent', 'dispositivo', 'localizacao', 'sucesso', 'created_at']
    ordering = ['-created_at']
...
class SessaoAtivaAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['usuario', 'session_key', 'ip_address', 'dispositivo', 'navegador', 'ultimo_acesso', 'created_at']
