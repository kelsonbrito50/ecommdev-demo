...
class Usuario(AbstractUser):
    ...
    foto = models.ImageField(
        _("Foto"), upload_to="usuarios/fotos/", blank=True, null=True, validators=[validate_avatar]
    )
    ...
    email_verification_token = models.UUIDField(
        _("Token de Verificação"), default=uuid.uuid4, editable=False
    )
...
class PerfilEmpresa(models.Model):
    ...
    usuario = models.OneToOneField(
        Usuario, on_delete=models.CASCADE, related_name="empresa", verbose_name=_("Usuário")
    )
...
class LogLogin(models.Model):
    ...
    usuario = models.ForeignKey(
        Usuario, on_delete=models.CASCADE, related_name="logs_login", verbose_name=_("Usuário")
    )
...
class SessaoAtiva(models.Model):
    ...
    usuario = models.ForeignKey(
        Usuario, on_delete=models.CASCADE, related_name="sessoes_ativas", verbose_name=_("Usuário")
    )
