...
from .forms import AlterarSenhaForm, PerfilForm, RegistroForm
from .models import LogLogin, SessaoAtiva, Usuario
        ...
        if request.method == "POST":
            ...
            if self._is_account_locked(email):
                messages.error(
                    request,
                    _(
                        "Conta temporariamente bloqueada devido a muitas tentativas. Tente novamente em 30 minutos."
                    ),
                )
                form = self.get_form()
...
class RegistroView(RateLimitMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if email_sent:
            messages.success(
                self.request, _("Conta criada! Verifique seu email para ativar sua conta.")
            )
        else:
            messages.warning(
                self.request,
                _(
                    "Conta criada! Houve um problema ao enviar o email de verificação. Entre em contato conosco."
                ),
            )
        return response
...
class EncerrarSessaoView(LoginRequiredMixin, View):
    ...
    def post(self, request, pk):
        sessao = get_object_or_404(SessaoAtiva, pk=pk, usuario=request.user)
...
class VerificarEmailView(View):
    ...
    def get(self, request, token):
        ...
        if not user.is_verification_token_valid():
            messages.error(
                request, _("Este link de verificação expirou. Solicite um novo link abaixo.")
            )
...
class ReenviarVerificacaoView(RateLimitMixin, View):
    ...
    def post(self, request):
        ...
        generic_message = _(
            "Se este email estiver cadastrado e ainda não verificado, você receberá um link de verificação."
        )
