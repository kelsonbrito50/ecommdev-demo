...
class Servico(models.Model):
    ...
    # Pricing
    preco = models.DecimalField(_('Preço'), max_digits=10, decimal_places=2, default=Decimal('0.00'))
    preco_ate = models.DecimalField(_('Preço Até'), max_digits=10, decimal_places=2, null=True, blank=True)
    ...
    def get_preco_display(self):
        if self.tipo_preco == 'apartir':
            return f"A partir de R$ {self.preco:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')
        elif self.tipo_preco == 'mensal':
            return f"R$ {self.preco:,.2f}/mês".replace(',', 'X').replace('.', ',').replace('X', '.')
        elif self.tipo_preco == 'hora':
            return f"R$ {self.preco:,.2f}/hora".replace(',', 'X').replace('.', ',').replace('X', '.')
        else:
