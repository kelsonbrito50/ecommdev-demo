...
class Ticket(models.Model):
    ...
    prioridade = models.CharField(
        _("Prioridade"), max_length=20, choices=PRIORIDADE_CHOICES, default="media"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.numero:
            ...
            prefix = f"TKT-{year}-"
            last = Ticket.objects.filter(numero__startswith=prefix).aggregate(
                max_num=Max("numero")
            )["max_num"]
            if last:
...
class RespostaTicket(models.Model):
    ...
    ticket = models.ForeignKey(
        Ticket, on_delete=models.CASCADE, related_name="respostas", verbose_name=_("Ticket")
    )
...
class AvaliacaoTicket(models.Model):
    ...
    ticket = models.OneToOneField(
        Ticket, on_delete=models.CASCADE, related_name="avaliacao", verbose_name=_("Ticket")
    )
    nota = models.PositiveSmallIntegerField(_("Nota"), choices=[(i, str(i)) for i in range(1, 6)])
