...
class TicketAdmin(admin.ModelAdmin):
    list_display = ['numero', 'assunto', 'cliente', 'categoria', 'prioridade_badge', 'status_badge', 'atendente', 'created_at']
    list_filter = ['status', 'prioridade', 'categoria', 'created_at']
    search_fields = ['numero', 'assunto', 'descricao', 'cliente__email']
    readonly_fields = ['numero', 'created_at', 'updated_at', 'data_primeira_resposta', 'data_resolucao']
    ordering = ['-created_at']
    ...
    fieldsets = (
        ('Identificacao', {
            'fields': ('numero', 'status', 'cliente', 'atendente')
        }),
        ('Ticket', {
    ...
    def prioridade_badge(self, obj):
        ...
        return format_html('<span class="badge bg-{}">{}</span>', color, obj.get_prioridade_display())
