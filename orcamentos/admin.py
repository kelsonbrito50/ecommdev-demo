...
from .models import Orcamento, HistoricoOrcamento


class HistoricoOrcamentoInline(admin.TabularInline):
    model = HistoricoOrcamento
    extra = 0
    readonly_fields = ['usuario', 'acao', 'status_anterior', 'status_novo', 'observacao', 'created_at']
    can_delete = False
...
class OrcamentoAdmin(admin.ModelAdmin):
    list_display = ['numero', 'nome_completo', 'tipo_projeto', 'status_badge', 'valor_proposto_display', 'created_at']
    ...
    fieldsets = (
        ('Identificacao', {
            'fields': ('numero', 'status', 'cliente')
        }),
        ('Dados do Cliente', {
            'fields': ('nome_completo', 'email', 'telefone', 'empresa', 'cnpj', 'cidade', 'estado')
        }),
        ('Projeto', {
            'fields': ('tipo_projeto', 'pacote', 'descricao_projeto', 'objetivos', 'publico_alvo')
        }),
        ('Requisitos Tecnicos', {
            'fields': ('funcionalidades', 'integracoes', 'sistema_pagamento', 'referencia_design', 'possui_dominio', 'possui_hospedagem'),
            'classes': ('collapse',)
        }),
        ('Orcamento e Prazo', {
        ...
        ('Proposta', {
            'fields': ('valor_proposto', 'observacoes_internas')
        }),
        ('Metadados', {
...
class HistoricoOrcamentoAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ['orcamento', 'usuario', 'acao', 'status_anterior', 'status_novo', 'observacao', 'created_at']
    ordering = ['-created_at']
